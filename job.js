//CRON job to run should take care of refresh token concerns
//Also, go through playlists 
...
function refreshAccessToken(user) {
  ...
  //Refresh access token first thing, since this job will run in a regular interval e.g. 1 week.
  let res = axios({
    method: 'post',
...
//This job is the update the playlists with the currently listed filters every week. Need to store acces and refresh tokens for user.
  ...
  if (err) {
    ...
    if (users) {
      ...
      for (let i=0; i<users.length; i++) {
        promise = refreshAccessToken(users[i])
